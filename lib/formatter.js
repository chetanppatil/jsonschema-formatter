...
function validateSchema (data, schema, errorCodeObj) {
  let ERR_CODE
  if (errorCodeObj) { ERR_CODE = Object.assign(errorCodeObj) }
  ...
  return new Promise((resolve, reject) => {
    ...
    validationErr.length > 0 ? reject({ status: 'fail', errors: validationErr }) : resolve({ status: 'pass', message: 'No error' }) // eslint-disable-line
